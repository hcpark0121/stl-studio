...
import {labelPolygons} from '../coin-cell-box/labels.js';
...
for(const [i,design] of cases.entries()){
 ...
 const expected=L.blocks.reduce((s,block)=>s+labelArea(labelPolygons(block,TYPES[block.type].id))*.6,0);
